...
class indexV :
    ...
    def __str__ (self):
        if judgeInt(str(self.vertex.x)) :
            self.vertex.x = int(self.vertex.x)
        if judgeInt(str(self.vertex.y)) :
            self.vertex.y = int(self.vertex.y)
        return str(self.index) + ': (' + str(self.vertex.x) + ',' + str(self.vertex.y) + ')'
...
class Graph :
    ...
    def calculate(self,database) :
        ...
        for i in range(0,len(database.Streets) - 1) :
            for l1 in database.Streets[i].Lines :
                for l2 in database.Streets[i+1].Lines :
        ...
        for i in range(0,len(self.wholeEdges) - 1) :
            inPoint = intersect (self.wholeEdges[i],self.wholeEdges[i+1]) 
            if inPoint != None :
                eList[i].getPoints(inPoint)
                eList[i+1].getPoints(inPoint)
        for x in eList :
...
def getName(readline) :
    ...
    if isNameValid (name) == False : return False
    return name
...
def isNameValid (name):
    for x in name :
        if (not x.isalpha()) and (not x.isspace()):
            print('Error:The street name ',name, ' is invalid!')
            return False
    if name[0].isspace() or name[len(name) - 1].isspace()  :
        print('Error:The street name ',name, ' is invalid!')
        return False
    flag = 1
    for x in name :
...
def main():
    ...
    while True:
        line = sys.stdin.readline() #This is the input
        if line == "":
            break
